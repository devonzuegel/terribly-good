...
def validParts():
  ...
  partNames = [ 'Development All Words', \
                ...
                'Development - Binary Naive Bayes', \
                'Testing - Binary Naive Bayes'
              ]
  return partNames
...
def getChallenge(email, partId):
  ...
  url = challenge_url()
  assignmentPartSid = "%s-%d" % (homework_id(), partId)
  if DRAFT_SUBMISSION:
    assignmentPartSid += "-dev"
  print assignmentPartSid
  values = {'email_address' : email, 'assignment_part_sid' : assignmentPartSid, 'response_encoding' : 'delim'}
...
def submitSolution(email_address, ch_resp, part, output, source, state, ch_aux):
  ...
  assignmentPartSid = ("%s-%d" % (homework_id(), part))

  if DRAFT_SUBMISSION:
    assignmentPartSid += "-dev"

  values = { 'assignment_part_sid' : assignmentPartSid, \
...
def output(partId, ch_aux):
  ...
  elif partId == 5: # development boolean features
    splits = classifier.crossValidationSplits(trainDir)
    accuracy = 0.0
    for split in splits:
      nb = NaiveBayes()
  ...
  elif partId == 6: # testing boolean features
